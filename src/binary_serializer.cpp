...
void BinarySerializer::serialize_order(const Order& order, std::vector<uint8_t>& buffer){
    ...
    */
    constexpr size_t PAYLOAD_SIZE = sizeof(uint64_t) + sizeof(Price) 
    ...
    buffer.resize(offset + 5 + PAYLOAD_SIZE);
        ...
        std::memcpy(buffer.data() + offset, &price, sizeof(Price));
        offset+=sizeof(Price);
        //quantity
...
Order BinarySerializer::deserialize_order(const Message& message){
        ...
        offset+=sizeof(uint64_t);
        Price price;
        std::memcpy(&price, payload.data() + offset, sizeof(Price));
        offset+=sizeof(Price);
        //quantity
        uint32_t quantity;
...
void BinarySerializer::serialize_trade(const Trade& trade, std::vector<uint8_t>& buffer){
    constexpr size_t PAYLOAD_SIZE = sizeof(Price) + sizeof(uint64_t) + sizeof(uint64_t)
    ...
    std::memcpy(buffer.data() + offset, &trade.price, sizeof(Price));
    offset+=sizeof(Price);
...
Trade BinarySerializer::deserialize_trade(const Message& message){
    ...
    Price price;
    std::memcpy(&price, message.payload.data() + offset, sizeof(Price));
    offset+=sizeof(Price);
